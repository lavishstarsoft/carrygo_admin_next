...
export default function OrdersPage() {
    ...
    const handleDeleteConfirm = async () => {
        if (!deletePassword) {
            setDeleteError('Please enter authorization password.');
            return;
        }
        setIsDeleting(true);
        setDeleteError('');
        try {
            const res = await fetch(`${apiUrl}/orders/${deletingOrderId}`, {
                method: 'DELETE',
                headers: {
                    ...
                    <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-100 flex flex-col gap-4 animate-in zoom-in-95 duration-200">
                        <div>
                            <h3 className="text-lg font-black text-slate-800 tracking-tight flex items-center gap-2">
                                <span className="text-rose-500"><ShieldAlertIcon /></span>
                                Authorize Database Deletion
                            </h3>
                            <p className="text-slate-500 text-xs mt-2 font-medium">
                                You are permanently deleting order reference:
                                <code className="block bg-slate-50 border border-slate-100 text-slate-700 font-mono text-[10px] font-bold p-2 mt-1.5 rounded-lg">
                                    #{deletingOrderId}
                                </code>
                            </p>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                type="password"
                                placeholder="Enter DB Authorization Password"
                                value={deletePassword}
                                onChange={(e) => setDeletePassword(e.target.value)}
                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500/30 focus:border-rose-500 focus:bg-white transition-all shadow-inner"
                                autoFocus
                                onKeyDown={(e) => {
                            ...
                            />
                            {deleteError && (
                                <p className="text-rose-600 text-xs font-bold mt-1.5 bg-rose-50 border border-rose-100 p-2 rounded-lg">
                                    ⚠️ {deleteError}
                                </p>
                            )}
                        </div>
                        ...
                        <div className="flex gap-3 mt-2">
                            <button
                                ...
                                className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs uppercase tracking-wider rounded-xl transition-all disabled:opacity-50"
                            >
                                Abort
                            </button>
                            <button
                                ...
                                className="flex-1 px-4 py-3 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-rose-600/30 hover:shadow-rose-600/40 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                            >
                                {isDeleting ? (
                                    <>
